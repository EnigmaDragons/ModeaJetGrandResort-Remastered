...
namespace EnigmaDragons.NodeSystem
{
    public class NodeEditorMenuBar : IElement
    {
        ...
        public NodeEditorMenuBar(EditorWindow window, Action<string> onSave, Action onLoad, Action onNew,
            Action onCenter, Action onResetZoom)
        {
            _window = window;
            _onSave = onSave;
            _onLoad = onLoad;
        ...
        public void Draw(Vector2 position)
        {
            ...
            if (GUILayout.Button(new GUIContent("Save"), EditorStyles.toolbarButton,
                GUILayout.Width(EditorConstants.MenuBarButtonWidth)))
                _onSave(NodeTreeName);
            GUILayout.Space(5);
            if (GUILayout.Button(new GUIContent("Load"), EditorStyles.toolbarButton,
                GUILayout.Width(EditorConstants.MenuBarButtonWidth)))
                _onLoad();
            GUILayout.Space(5);
            if (GUILayout.Button(new GUIContent("New"), EditorStyles.toolbarButton,
                GUILayout.Width(EditorConstants.MenuBarButtonWidth)))
                _onNew();
            GUILayout.Space(5);
            if (GUILayout.Button(new GUIContent("Center"), EditorStyles.toolbarButton,
                GUILayout.Width(EditorConstants.MenuBarButtonWidth)))
                _onCenter();
            if (GUILayout.Button(new GUIContent("Reset Zoom"), EditorStyles.toolbarButton,
                GUILayout.Width(EditorConstants.MenuBarButtonWidth)))
                _onResetZoom();
            NodeTreeName = GUILayout.TextField(NodeTreeName, EditorStyles.textField);
