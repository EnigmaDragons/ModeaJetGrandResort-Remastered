...
namespace EnigmaDragons.NodeSystem.Editor
{
    public class NodeContent
    {
        ...
        public NodeContent(GuidToTypeMap map, NodeData nodeData) : this(map[nodeData.TypeGuid], nodeData.Properties) { }
        ...
        public NodeContent(Type type, Dictionary<string, string> properties)
        {
            ...
            type.GetProperties().Where(x => x.CanWrite).ToArray().ForEach(prop =>
            {
                ...
                {
                    _properties[prop.Name] = false.ToString();
                ...
                {
                    _properties[prop.Name] = 0.ToString();
                ...
                {
                    _properties[prop.Name] = 0f.ToString();
                    _elements.Add(new ElementLabel(new ExpandingTextField(_properties[prop.Name], num =>
                    {
                ...
                else if (typeof(ScriptableObject).IsAssignableFrom(prop.PropertyType))
                {
                    ...
                    _properties[prop.Name] = items.First().GetInstanceID().ToString();
        ...
        public NodeData Save() => new NodeData { TypeGuid = _type.GUID.ToString(), Properties = _properties };
    }
}